        ...
        if self.displayed_name in ('GE/Carescape', 'Philips/IntelliVue'):
            if 'NIBP_SBP' in col_dict.keys() and 'NIBP_DBP' in col_dict.keys() and 'NIBP_MBP' in col_dict.keys():
                for i in range(len(val)):
                    if np.isnan(val[i, col_dict['NIBP_SBP']]) or np.isnan(val[i, col_dict['NIBP_DBP']]) or np.isnan(
                            val[i, col_dict['NIBP_MBP']]):
                        val[i, col_dict['NIBP_SBP']] = val[i, col_dict['NIBP_DBP']] = val[
                            i, col_dict['NIBP_MBP']] = np.nan
            ...
            if 'ABP_SBP' in col_dict.keys() and 'ABP_DBP' in col_dict.keys() and 'ABP_MBP' in col_dict.keys():
                for i in range(len(val)):
                    if np.isnan(val[i, col_dict['ABP_SBP']]) or np.isnan(val[i, col_dict['ABP_DBP']]) or np.isnan(
                            val[i, col_dict['ABP_MBP']]):
...
class FileRecorded(models.Model):
    ...
    class Meta:
        unique_together = (("begin_date", "file_basename"),)
        ...
        for i in range(len(r)):
            ...
            if side == 1:
                ...
                if type == 'unixtime':
                    ...
                    while timestamp[p_end] - timestamp[i] <= datetime.timedelta(seconds=windowsize) if p_end < len(
                            r) else False:
                        p_end += 1
                else:
                    assert False, 'Unknown timestamp type.'
        ...
        with connection.cursor() as cursor:
            ...
            for i in range(len(timestamp)):
                if current_row_count:
                    ...
                    query = "INSERT IGNORE INTO %s (%s) VALUES " % (
                    self.device.db_table_name, ', '.join(column_info_db))
...
class NumberPIV(models.Model):
    ...
    # patients_id = models.IntegerField(db_index=True, null=True)
...
# Trend calculated Table
class Count_info(models.Model):
    type = models.CharField(max_length=10)
    year = models.CharField(max_length=10, null=True)
    collected_files = models.IntegerField(null=True)
    collected_hours = models.FloatField(null=True)
    patients_saved = models.IntegerField(null=True)
    count_bx50 = models.IntegerField(null=True)
    count_bis = models.IntegerField(null=True)
    count_ev1000 = models.IntegerField(null=True)
    count_dataq = models.IntegerField(null=True)
    count_invos = models.IntegerField(null=True)
    count_root = models.IntegerField(null=True)
    count_intellivue = models.IntegerField(null=True)
    count_primus = models.IntegerField(null=True)
    count_vigilance = models.IntegerField(null=True)
...
class operation(models.Model):
    ...
    class Meta:
        unique_together = ("room", "begin_date", "end_date")
...
class operation_record(models.Model):
    ...
    class Meta:
        unique_together = ("operation_id", "record_id")


class check_save_operation(models.Model):
    operation = models.ForeignKey('operation', on_delete=models.CASCADE)
    device = models.ForeignKey('device', on_delete=models.CASCADE)
    channel = models.CharField(max_length=64, null=True,db_index=True)
    count_file = models.IntegerField(null=True)
    graph_filepath = models.CharField(max_length=64, null=True)
    surgery_time = models.IntegerField(null=True)
    exist_time = models.IntegerField(null=True)
    save_percent = models.IntegerField(null=True)

    class Meta:
        unique_together = ("operation_id", "device_id", 'channel')


class bed_status_record(models.Model):
    date = models.DateTimeField(auto_now=True)
    bed = models.CharField(max_length=32, null=True)
    status = models.CharField(max_length=16, null=True)

    class Meta:
        unique_together = ("date", "bed")
